                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <div class="header-title">
                            <h4 class="card-title">Preview Surat Keterangan Meninggal Dunia</h4>
                            <p class="mb-0 text-muted">{{ $skkm->nama_almarhum }} -
                                {{ $suratTerbit->nomor_surat ?? 'Belum ada nomor' }}</p>
                        </div>
                        <div class="d-flex gap-2">
                            <a href="{{ route('skkm.index') }}" class="btn btn-secondary">
                                <i class="ri-arrow-left-line"></i> Kembali
                            </a>
                            @if ($suratTerbit)
                                <a href="{{ route('skkm.print', $skkm->id) }}" class="btn btn-primary" target="_blank">
                            ...
                            <div class="surat-container">
                                ...
                                <div class="content-surat">
                                    ...
                                    <p>
                                        ...
                                        atas benar penduduk Jorong <span
                                            class="dot-line">{{ $skkm->alamat_almarhum }}</span> Nagari Koto
                                        Alam yang telah meninggal dunia pada tanggal
                                        <span
                                            class="dot-line">{{ \Carbon\Carbon::parse($skkm->tanggal_meninggal)->translatedFormat('d') }}</span>
                                        bulan
                                        <span
                                            class="dot-line">{{ \Carbon\Carbon::parse($skkm->tanggal_meninggal)->translatedFormat('F') }}</span>
                                        tahun
                                        <span
                                            class="dot-line">{{ \Carbon\Carbon::parse($skkm->tanggal_meninggal)->translatedFormat('Y') }}</span>,
                                        di
                                        Nagari Koto Alam.
                                    </p>
                                ...
                                <div class="signature-section">
                                    <div class="signature-right">
                                        ...
                                        Pada Tanggal : <span
                                            class="dot-line">{{ \Carbon\Carbon::parse($suratTerbit->tanggal_terbit ?? now())->translatedFormat('d F Y') }}</span><br /><br />
                                ...
                                <div class="col-md-6">
                                    ...
                                    <table class="table table-sm table-borderless">
                                        <tr>
                                            ...
                                            <td>:</td>
                                            <td>
                                                @switch($skkm->permintaanSurat->status)
                                                    @case('Diproses')
                                                        <span class="badge bg-warning">{{ $skkm->permintaanSurat->status }}</span>
                                                    @break

                                                    @case('Diterima')
                                                        <span class="badge bg-success">{{ $skkm->permintaanSurat->status }}</span>
                                                    @break

                                                    @case('Ditolak')
                                                        <span class="badge bg-danger">{{ $skkm->permintaanSurat->status }}</span>
                                                    @break

                                                    @default
                                                        <span
                                                            class="badge bg-secondary">{{ $skkm->permintaanSurat->status }}</span>
                                                @endswitch
                                            </td>
                                        </tr>
                                        <tr>
                                            ...
                                            <td>:</td>
                                            <td>{{ \Carbon\Carbon::parse($skkm->permintaanSurat->tanggal_permintaan)->format('d F Y') }}
                                            </td>
                                        </tr>
                                        @if ($suratTerbit)
                                            <tr>
                                                ...
                                                <td>:</td>
                                                <td>{{ \Carbon\Carbon::parse($suratTerbit->tanggal_terbit)->format('d F Y H:i') }}
                                                </td>
                                            </tr>
                                        @endif
                                    </table>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <table class="table table-sm table-borderless">
                                        <tr>
                                            ...
                                            <td>:</td>
                                            <td>{{ $skkm->permintaanSurat->verifikasiPengguna->nama_lengkap }}</td>
                                        </tr>
                                        <tr>
                                            <td width="150">Keperluan</td>
                                            <td>:</td>
                                            <td>{{ $skkm->keperluan }}</td>
                                        </tr>
                                        <tr>
                                            <td>Jenis Surat</td>
                                            <td>:</td>
                                            <td>{{ $skkm->permintaanSurat->jenisSurat->nama_surat ?? 'SKKM' }}</td>
                                        </tr>
                                    </table>
                                </div>
                            </div>
