...
if ($.fn.DataTable) {
    if ($('[data-toggle="data-table"]').length) {
        $('[data-toggle="data-table"]').each(function () {
            const table = $(this);
            // Check if table has thead and tbody with matching column counts
            const theadColumns = table.find("thead tr:first th").length;
            const tbodyColumns = table.find("tbody tr:first td").length;

            // Only initialize DataTable if column counts match or tbody is empty
            if (
                theadColumns > 0 &&
                (tbodyColumns === theadColumns || tbodyColumns === 0)
            ) {
                try {
                    table.DataTable({
                    ...
                    console.warn(
                        "DataTable initialization failed for table:",
                        this,
                        error
                    );
                }
